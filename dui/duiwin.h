...
#include "duibase.h"
#include "duitrace.h"
#include "duistdthunk.h"

namespace DUI
{
	/////////////////////////////////////////////////////////////////////////////
	// DWinTraits - Defines various default values for a window
	...
	/////////////////////////////////////////////////////////////////////////////
	// DMessageMap - abstract class that provides an interface for message maps

	class DUI_NO_VTABLE DMessageMap
	{
	public:
	...
	/////////////////////////////////////////////////////////////////////////////
	// _DUI_MSG - extended MSG structure
	...
	public:
		...
		// Constructors
		_DUI_MSG() : cbSize(sizeof(_DUI_MSG)), bHandled(TRUE)
		{
			hwnd = NULL;
			message = 0;
			wParam = 0;
		...
		_DUI_MSG(
			...
			cbSize(sizeof(_DUI_MSG)), bHandled(bHandledIn)
		{
			hwnd = hWnd;
			message = uMsg;
			wParam = wParamIn;
	...
	/////////////////////////////////////////////////////////////////////////////
	// DWindow - client side for a Windows window
	...
	public:
		...
		DWindow(_In_opt_ HWND hWnd = NULL) throw() :
			m_hWnd(hWnd)
		{
		}

		DWindow& operator=(_In_opt_ HWND hWnd) throw()
		{
			m_hWnd = hWnd;
			return *this;
		}
		...
		{
			DUIASSUME(m_hWnd == NULL);
			DUIASSERT((hWndNew == NULL) || ::IsWindow(hWndNew));
			m_hWnd = hWndNew;
		}
			...
			_Inout_ _Outref_result_maybenull_ _Post_z_ BSTR& bstrText) const throw()
		{
			...
			return DWindow(hWndCtl).GetWindowText(bstrText);
		}
#endif // _OLEAUTO_H_
		DWindow GetNextDlgGroupItem(
			_In_ HWND hWndCtl,
			_In_ BOOL bPrevious = FALSE) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetNextDlgGroupItem(m_hWnd, hWndCtl, bPrevious));
		}

		DWindow GetNextDlgTabItem(
			_In_ HWND hWndCtl,
			_In_ BOOL bPrevious = FALSE) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetNextDlgTabItem(m_hWnd, hWndCtl, bPrevious));
		}
		...
		DWindow ChildWindowFromPoint(_In_ POINT point) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::ChildWindowFromPoint(m_hWnd, point));
		}

		DWindow ChildWindowFromPointEx(
			_In_ POINT point,
			_In_ UINT uFlags) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::ChildWindowFromPointEx(m_hWnd, point, uFlags));
		}

		DWindow GetTopWindow() const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetTopWindow(m_hWnd));
		}

		DWindow GetWindow(_In_ UINT nCmd) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetWindow(m_hWnd, nCmd));
		}

		DWindow GetLastActivePopup() const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetLastActivePopup(m_hWnd));
		}
		...
		DWindow GetParent() const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetParent(m_hWnd));
		}

		DWindow SetParent(_In_ HWND hWndNewParent) throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::SetParent(m_hWnd, hWndNewParent));
		}
		...
		DWindow GetDlgItem(_In_ int nID) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			return DWindow(::GetDlgItem(m_hWnd, nID));
		}
		...
		{
			DUIASSERT(::IsWindow(m_hWnd)); ::DragAcceptFiles(m_hWnd, bAccept);
		}
#endif
		...
		DWindow GetDescendantWindow(_In_ int nID) const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			...
			if ((hWndChild = ::GetDlgItem(m_hWnd, nID)) != NULL)
			{
				if (::GetTopWindow(hWndChild) != NULL)
				{
					...
					DWindow wnd(hWndChild);
					hWndTmp = wnd.GetDescendantWindow(nID);
					if (hWndTmp != NULL)
						return DWindow(hWndTmp);
				}
				return DWindow(hWndChild);
			}
		...
		ATLPREFAST_UNSUPPRESS()
#endif // _OLEAUTO_H_
		DWindow GetTopLevelParent() const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
			...
			return DWindow(hWndParent);
		}

		DWindow GetTopLevelWindow() const throw()
		{
			DUIASSERT(::IsWindow(m_hWnd));
	...
	_declspec(selectany) RECT DWindow::rcDefault = { CW_USEDEFAULT, CW_USEDEFAULT, 0, 0 };

	/////////////////////////////////////////////////////////////////////////////
	// DContainedWindow - Implements a contained window

	template <class TBase = DWindow, class TWinTraits = DControlWinTraits >
	class DContainedWindowT :
		public TBase
	{
	public:
		DWndProcThunk m_thunk;
		LPCTSTR m_lpszClassName;
		WNDPROC m_pfnSuperWindowProc;
		DMessageMap* m_pObject;
		DWORD m_dwMsgMapID;
		const _DUI_MSG* m_pCurrentMsg;

		// If you use this constructor you must supply
		// the Window Class Name, Object* and Message Map ID
		// Later to the Create call
		DContainedWindowT() : m_pCurrentMsg(NULL)
		{
		}

		DContainedWindowT(
			_In_z_ LPCTSTR lpszClassName,
			_In_ DMessageMap* pObject,
			_In_ DWORD dwMsgMapID = 0)
		...
		const _DUI_MSG* GetCurrentMessage() const
		{
		...
		{
			const _DUI_MSG* pMsg = m_pCurrentMsg;
			LRESULT lRes = 0;
			if (pMsg != NULL)
		...
		static LRESULT CALLBACK StartWindowProc(
			...
			_In_ LPARAM lParam)
		{
			DContainedWindowT< TBase >* pThis = (DContainedWindowT< TBase >*)_AtlWinModule.ExtractCreateWndData();
			...
			if (pOldProc != StartWindowProc)
				DUITRACE(atlTraceWindowing, 0, _T("Subclassing through a hook discarded.\n"));
#else
		...
		static LRESULT CALLBACK WindowProc(
			...
			_In_ LPARAM lParam)
		{
			DContainedWindowT< TBase >* pThis = (DContainedWindowT< TBase >*)hWnd;
			DUIASSERT(pThis);
			if (!pThis)
			{
				return 0;
			}
			DUIASSERT(pThis->m_hWnd != NULL);
			DUIASSERT(pThis->m_pObject != NULL);
			if (!pThis->m_hWnd || !pThis->m_pObject)
			{
				return 0;
			}
			// set a ptr to this message and save the old value
			_DUI_MSG msg(pThis->m_hWnd, uMsg, wParam, lParam);
			const _DUI_MSG* pOldMsg = pThis->m_pCurrentMsg;
	...
	typedef DContainedWindowT<DWindow, DControlWinTraits>	DContainedWindow;

	/////////////////////////////////////////////////////////////////////////////
	// DWindowImpl - Implements a window

	template <class TBase = DWindow >
	class ATL_NO_VTABLE DWindowImplRoot :
		public TBase,
		public DMessageMap
	{
	public:
		...
		DWindowImplRoot() : m_pCurrentMsg(NULL), m_dwState(0)
		{
		}

		virtual ~DWindowImplRoot()
		{
#ifdef _DEBUG
			if (this->m_hWnd != NULL)	// should be cleared in WindowProc
			{
				DUITRACE(atlTraceWindowing, 0, _T("ERROR - Object deleted before window was destroyed\n"));
				DUIASSERT(FALSE);
			}
#endif //_DEBUG
		}
		...
		{
			const _DUI_MSG* pMsg = GetCurrentMessage();
			DUIASSUME(pMsg != NULL);
			DUIASSERT(pMsg->cbSize >= sizeof(_DUI_MSG));
			return pMsg->bHandled;
		}
		void SetMsgHandled(_In_ BOOL bHandled)
		{
			_DUI_MSG* pMsg = (_DUI_MSG*)GetCurrentMessage();	// override const
			DUIASSUME(pMsg != NULL);
			DUIASSERT(pMsg->cbSize >= sizeof(_DUI_MSG));
	...
	template <class TBase = DWindow, class TWinTraits = DControlWinTraits>
	class ATL_NO_VTABLE DWindowImplBaseT :
		public DWindowImplRoot< TBase >
	{
	public:
		WNDPROC m_pfnSuperWindowProc;

		DWindowImplBaseT() : m_pfnSuperWindowProc(::DefWindowProc)
		{
		}
		...
		BOOL DestroyWindow()
		{
			...
			DUIASSERT(::IsWindow(this->m_hWnd));
#endif
		...
		LRESULT DefWindowProc()
		{
			const _DUI_MSG* pMsg = this->m_pCurrentMsg;
			LRESULT lRes = 0;
			if (pMsg != NULL)
				lRes = DefWindowProc(pMsg->message, pMsg->wParam, pMsg->lParam);
			return lRes;
	...
	typedef DWindowImplBaseT<DWindow>	DWindowImplBase;

	template <class TBase, class TWinTraits>
	LRESULT CALLBACK DWindowImplBaseT< TBase, TWinTraits >::StartWindowProc(
		_In_ HWND hWnd,
		_In_ UINT uMsg,
		_In_ WPARAM wParam,
		_In_ LPARAM lParam)
	{
		DWindowImplBaseT< TBase, TWinTraits >* pThis = (DWindowImplBaseT< TBase, TWinTraits >*)_AtlWinModule.ExtractCreateWndData();
		DUIASSERT(pThis != NULL);
		if (!pThis)
		{
			return 0;
		...
		// Initialize the thunk.  This is allocated in DWindowImplBaseT::Create,
		// so failure is unexpected here.
	...
	template <class TBase, class TWinTraits>
	LRESULT CALLBACK DWindowImplBaseT< TBase, TWinTraits >::WindowProc(
		_In_ HWND hWnd,
		_In_ UINT uMsg,
		_In_ WPARAM wParam,
		_In_ LPARAM lParam)
	{
		DWindowImplBaseT< TBase, TWinTraits >* pThis = (DWindowImplBaseT< TBase, TWinTraits >*)hWnd;
		// set a ptr to this message and save the old value
		_DUI_MSG msg(pThis->m_hWnd, uMsg, wParam, lParam);
		const _DUI_MSG* pOldMsg = pThis->m_pCurrentMsg;
		pThis->m_pCurrentMsg = &msg;
		...
		if (!bRet)
		{
			...
			else
			{
				...
				pThis->m_dwState |= DWindowImplRoot<TBase>::WINSTATE_DESTROYED;
			}
		}
		if ((pThis->m_dwState & DWindowImplRoot<TBase>::WINSTATE_DESTROYED) && pOldMsg == NULL)
		{
			// clear out window handle
	...
	template <class TBase, class TWinTraits>
	HWND DWindowImplBaseT< TBase, TWinTraits >::Create(
		_In_opt_ HWND hWndParent,
		_In_ _U_RECT rect,
		_In_z_ LPCTSTR szWindowName,
		_In_ DWORD dwStyle,
		...
		_In_opt_ LPVOID lpCreateParam)
	{
		...
		_AtlWinModule.AddCreateWndData((ATL::_AtlCreateWndData*)(&this->m_thunk.cd), this);
	...
	template <class TBase, class TWinTraits>
	BOOL DWindowImplBaseT< TBase, TWinTraits >::SubclassWindow(_In_ HWND hWnd)
	{
		DUIASSUME(this->m_hWnd == NULL);
		DUIASSERT(::IsWindow(hWnd));
	...
	// WindowProc will automatically subclass when  window goes away
	template <class TBase, class TWinTraits>
	HWND DWindowImplBaseT< TBase, TWinTraits >::UnsubclassWindow(_In_ BOOL bForce /*= FALSE*/)
	{
		DUIASSUME(this->m_hWnd != NULL);
	...
	template <class T, class TBase = DWindow , class TWinTraits = DControlWinTraits >
	class ATL_NO_VTABLE DWindowImpl :
		public DWindowImplBaseT< TBase, TWinTraits >
	{
	public:
		DECLARE_WND_CLASS2(NULL, DWindowImpl)
			...
			_In_opt_ LPVOID lpCreateParam = NULL)
		{
			...
			return DWindowImplBaseT< TBase, TWinTraits >::Create(hWndParent, rect, szWindowName,
...
class DWindow;
#ifndef _ATL_NO_HOSTING
template <class TBase = DWindow> class CAxWindowT;
template <class TBase = DWindow> class CAxWindow2T;
#endif //!_ATL_NO_HOSTING
class DMessageMap;
class CDynamicChain;
...
typedef CWinTraits<WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0>									DControlWinTraits;
typedef CWinTraits<WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_APPWINDOW | WS_EX_WINDOWEDGE>		CFrameWinTraits;
typedef CWinTraits<WS_OVERLAPPEDWINDOW | WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_MDICHILD>	CMDIChildWinTraits;

typedef CWinTraits<0, 0> CNullTraits;

template <class T, class TBase = DWindow, class TWinTraits = DControlWinTraits> class DWindowImpl;
template <class T, class TBase = DWindow, class TWinTraits = DControlWinTraits> class CWindowWithReflectorImpl;
template <class T, class TBase = DWindow> class CDialogImpl;
#ifndef _ATL_NO_HOSTING
template <class T, class TBase = DWindow> class CAxDialogImpl;
#endif //!_ATL_NO_HOSTING
template <WORD t_wDlgTemplateID, BOOL t_bCenter = TRUE> class CSimpleDialog;
template <class TBase = DWindow, class TWinTraits = DControlWinTraits> class DContainedWindowT;
...
// Operations
	HWND Create(
		...
		_In_opt_ LPVOID lpCreateParam = NULL)
	{
		return DWindow::Create(GetWndClassName(), hWndParent, rect, szWindowName, dwStyle, dwExStyle, MenuOrID, lpCreateParam);
	}
		...
		_Inout_opt_ IUnknown* punkSink = NULL)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (ppUnkContainer)
			{
				...
				DUIASSERT(SUCCEEDED(hr)); // This should not fail!
			}
		}
...
typedef CAxWindowT<DWindow> CAxWindow;
...
typedef CAxWindow2T<DWindow> CAxWindow2;
...
class ATL_NO_VTABLE DMessageMap
{
public:
	virtual BOOL ProcessWindowMessage(
		_In_ HWND hWnd,
...
#define END_MSG_MAP() \
			break; \
		default: \
			DUITRACE(static_cast<int>(ATL::atlTraceWindowing), 0, _T("Invalid message map ID (%i)\n"), dwMsgMapID); \
			DUIASSERT(FALSE); \
			break; \
		} \
		return FALSE; \
...
LRESULT DWindowImplRoot< TBase >::ForwardNotifications(
	_In_ UINT uMsg,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam,
	_Out_ BOOL& bHandled)
...
ATLPREFAST_SUPPRESS(6101)
template <class TBase>
LRESULT DWindowImplRoot< TBase >::ReflectNotifications(
	_In_ UINT uMsg,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam,
	_Out_ BOOL& bHandled)
...
template <class TBase>
_Success_(return != FALSE) BOOL DWindowImplRoot< TBase >::DefaultReflectionHandler(
	_In_ HWND hWnd,
	_In_ UINT uMsg,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam,
...
class ATL_NO_VTABLE CWindowWithReflectorImpl :
	public DWindowImpl< T, TBase, TWinTraits >
{
public:
	HWND Create(
		_In_opt_ HWND hWndParent,
		...
		_In_opt_ LPVOID lpCreateParam = NULL)
	{
		...
		return DWindowImpl< T, TBase, TWinTraits >::Create(m_wndReflector, rcPos, szWindowName, dwStyle, dwExStyle, MenuOrID, lpCreateParam);
	}
	...
	_In_ LPARAM lParam)
{
	...
	if(bRet)
	{
		...
		default:
			...
			if((pThis->m_dwState & DWindowImplRoot<TBase>::WINSTATE_DESTROYED) == 0)
			{
	...
	if((pThis->m_dwState & DWindowImplRoot<TBase>::WINSTATE_DESTROYED) && pThis->m_pCurrentMsg == NULL)
	{
		// clear out window handle
		HWND hWndThis = pThis->m_hWnd;
		pThis->m_hWnd = NULL;
...
typedef CDialogImplBaseT<DWindow>	CDialogImplBase;

template <class T, class TBase /* = DWindow */>
...
public:
	...
	BOOL EndDialog(_In_ int nRetCode)
	{
		DUIASSERT(::IsWindow(this->m_hWnd));
#ifdef _DEBUG
		DUIASSUME(m_bModal);	// must be a modal dialog
#endif //_DEBUG
	...
	BOOL DestroyWindow()
	{
		DUIASSERT(::IsWindow(this->m_hWnd));
#ifdef _DEBUG
		DUIASSERT(!m_bModal);	// must not be a modal dialog
#endif //_DEBUG
...
// Event handling support and Message map
	HRESULT AdviseSinkMap(_In_ bool bAdvise)
	{
		if(!bAdvise && this->m_hWnd == NULL)
		{
			...
			DUITRACE(atlTraceControls, 1, _T("CAxDialogImpl::AdviseSinkMap called after the window was destroyed\n"));
			return S_OK;
		}
...
typedef DContainedWindowT<DWindow>	DContainedWindow;
...
inline void AtlGetDialogSize(
	...
	_In_ bool bPropertyPage = false)
{
	DUIASSERT(pTemplate != NULL);
	DUIASSERT(pSize != NULL);
	...
	_Inout_ T)
{
	...
	{
		DUITRACE(atlTraceWindowing, 0, _T("ERROR : Invalid Arguments to AtlModuleRegisterWndClassInfoT\n"));
		DUIASSERT(0);
		return 0;
	}
	...
	if (p->m_atom == 0)
	{
		...
		if (FAILED(lock.Lock()))
		{
			DUITRACE(atlTraceWindowing, 0, _T("ERROR : Unable to lock critical section in AtlModuleRegisterWndClassInfoT\n"));
			DUIASSERT(0);
			return 0;
		}
		if(p->m_atom == 0)
		{
			if (p->m_lpszOrigName != NULL)
			{
				...
				if(!T::GetClassInfoEx(NULL, p->m_lpszOrigName, &wc))
				{
					...
					{
						DUITRACE(atlTraceWindowing, 0, _T("ERROR : Could not obtain Window Class information for %Ts\n"), p->m_lpszOrigName);
						return 0;
					}
				}
				p->m_wc = wc;
