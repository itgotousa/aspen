#ifndef __DUIHOST_H__
#define __DUIHOST_H__
...
ATLPREFAST_UNSUPPRESS()
	...
	STDMETHOD(SetSite)(_Inout_opt_ IUnknown* pUnkSite)
	{
		...
		if (SUCCEEDED(hr) && this->m_spUnkSite)
		{
			// Look for "outer" IServiceProvider
			hr = this->m_spUnkSite->QueryInterface(__uuidof(IServiceProvider), (void**)&m_spServices);
	...
	STDMETHOD(GetContainer)(_Outptr_ IOleContainer** ppContainer)
	{
		...
		if (ppContainer)
		{
			...
			(*ppContainer) = NULL;
			if (this->m_spUnkSite)
				hr = this->m_spUnkSite->QueryInterface(__uuidof(IOleContainer), (void**)ppContainer);
			if (FAILED(hr))
